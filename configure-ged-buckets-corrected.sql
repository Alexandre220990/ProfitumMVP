...
-- Date: 2025-01-03
-- Version: 2.0 - Corrigé pour les tables existantes

-- ===== 1. VÉRIFICATION ET CRÉATION DES TABLES MANQUANTES =====

-- Vérifier si la table DocumentActivity existe, sinon la créer
...
-- Vérifier si la table DocumentShare existe, sinon la créer
...
-- ===== 4. POLITIQUES POUR LE BUCKET CLIENT-DOCUMENTS =====
...
CREATE POLICY "client_documents_view_own" ON storage.objects
    FOR SELECT USING (
        ...
        AND (
            -- Client propriétaire du document (basé sur le chemin)
...
CREATE POLICY "client_documents_expert_view" ON storage.objects
    FOR SELECT USING (
        ...
        AND EXISTS (
            ...
            JOIN "DocumentFile" df ON df.client_id = (
                SELECT cpe.client_id FROM "ClientProduitEligible" cpe 
                WHERE cpe.id = ea.client_produit_eligible_id
            )
...
-- ===== 5. POLITIQUES POUR LE BUCKET EXPERT-DOCUMENTS =====
...
-- ===== 6. POLITIQUES POUR LE BUCKET ADMIN-DOCUMENTS =====
...
-- ===== 7. POLITIQUES POUR LE BUCKET CHARTES-SIGNATURES =====
...
-- ===== 8. POLITIQUES POUR LE BUCKET RAPPORTS-AUDIT =====
...
-- ===== 9. POLITIQUES DE PARTAGE DE DOCUMENTS =====
...
CREATE POLICY "shared_document_access" ON storage.objects
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM "DocumentFile" df
            JOIN "ExpertAssignment" ea ON ea.client_produit_eligible_id IN (
                SELECT cpe.id FROM "ClientProduitEligible" cpe WHERE cpe.client_id = df.client_id
            )
...
CREATE INDEX IF NOT EXISTS idx_documentfile_expert_bucket 
ON "DocumentFile" (expert_id, bucket_name);
...
CREATE INDEX IF NOT EXISTS idx_expertassignment_client_produit_expert 
ON "ExpertAssignment" (client_produit_eligible_id, expert_id);

-- Index sur DocumentActivity pour les logs
...
-- Index sur DocumentShare pour les partages
...
BEGIN
    ...
    RAISE NOTICE '📋 Tables DocumentActivity et DocumentShare créées si nécessaire';
END $$; 