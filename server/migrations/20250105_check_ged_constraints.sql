-- ============================================================================
-- VÉRIFICATION DES CONTRAINTES GEDDOCUMENT
-- ============================================================================
...
-- 3. Vérifier les valeurs autorisées pour category
SELECT 
    'GED_ALLOWED_VALUES' as check_type,
    'category' as column_name,
    unnest(enum_range(NULL::text)) as allowed_value
FROM (
    ...
    WHERE enumtypid = (
        SELECT oid 
        FROM pg_type 
        WHERE typname = 'ged_document_category'
    )
    LIMIT 1
) AS enum_values;

-- 4. Test avec une valeur autorisée
...
SELECT 
    ...
    'business', -- Utiliser une valeur autorisée
