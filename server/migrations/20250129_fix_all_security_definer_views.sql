-- ============================================================================
-- CORRECTION COMPLÈTE : Retirer SECURITY DEFINER de toutes les vues
-- ============================================================================
-- Problème : Les vues avec SECURITY DEFINER contournent les politiques RLS
--            et les permissions de l'utilisateur qui interroge la vue
-- Solution : Recréer automatiquement toutes les vues avec SECURITY DEFINER 
--            sans cette propriété (utiliser SECURITY INVOKER par défaut)
-- ============================================================================
-- Date : 2025-01-29
-- ============================================================================
...
-- Fonction helper robuste pour recréer une vue sans SECURITY DEFINER
CREATE OR REPLACE FUNCTION recréer_vue_sans_security_definer(
  nom_vue TEXT, 
  schema_nom TEXT DEFAULT 'public'
)
RETURNS void AS $$
DECLARE
  definition TEXT;
...
BEGIN
  ...
  -- Récupérer la définition de la vue (sans les options de sécurité)
  BEGIN
  ...
  BEGIN
    -- Essayer d'abord sans CASCADE
    EXECUTE format('DROP VIEW IF EXISTS %I.%I', schema_nom, nom_vue);
  EXCEPTION WHEN OTHERS THEN
    -- Si échec, essayer avec CASCADE
    BEGIN
  ...
  -- Nettoyer la définition (supprimer toute référence à SECURITY DEFINER)
  ...
  definition := REPLACE(definition, 'security definer', '');
  definition := REPLACE(definition, 'SECURITY_DEFINER', '');
  definition := REPLACE(definition, 'security_definer', '');
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    -- Tenter de restaurer l'ancienne vue si possible (mais sans SECURITY DEFINER)
    RETURN;
  END;
END;
$$ LANGUAGE plpgsql;

-- Liste complète de toutes les vues à corriger (basée sur le rapport du linter)
DO $$
DECLARE
  vue TEXT;
  vues TEXT[] := ARRAY[
...
BEGIN
  ...
  -- Vérifier aussi dans les définitions (pour les vues créées différemment)
  FOR vue_record IN
    SELECT 
    ...
    WHERE schemaname = 'public'
      ...
      AND NOT EXISTS (
        ...
        JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname = 'public'
          AND c.relname = vue_record.view_name
          AND c.relkind = 'v'
          AND (c.reloptions IS NULL OR 'security_definer=true' != ALL(c.reloptions))
      )
  LOOP
    BEGIN
      PERFORM recréer_vue_sans_security_definer(vue_record.view_name, vue_record.schema_name);
...
BEGIN
  ...
  IF nb_vues_restantes > 0 THEN
    ...
    RAISE WARNING '   Veuillez vérifier manuellement ces vues.';
  ELSE
    RAISE NOTICE '✅ Toutes les vues ont été corrigées avec succès!';
    RAISE NOTICE '✅ Aucune vue avec SECURITY DEFINER restante.';
    RAISE NOTICE '✅ Les politiques RLS seront maintenant correctement appliquées.';
  END IF;
