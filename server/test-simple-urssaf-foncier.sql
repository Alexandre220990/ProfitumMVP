-- =====================================================
-- TEST SIMPLIFIÉ URSSAF ET FONCIER
-- =====================================================
...
-- 3. Tester l'insertion directe de documents (sans créer de dossier)
DO $$
DECLARE
...
BEGIN
    ...
    -- Créer la relation document-label (réutilisation du label KBIS)
    INSERT INTO "GEDDocumentLabelRelation" (
        document_id,
        label_id,
        created_at
...
-- 4. Vérifier les documents créés
SELECT 
    'DOCUMENTS_CRÉÉS' as check_type,
    gd.id,
    gd.title,
    gd.category,
    gd.file_path,
...
-- 5. Vérifier la réutilisation du KBIS
SELECT 
    'RÉUTILISATION_KBIS' as check_type,
    gd.id,
    gd.title,
    gd.category,
    gd.content,
...
-- 7. Nettoyer les tests (optionnel)
