-- Script corrigé pour créer les conversations admin
-- Utilise la table Admin au lieu de authenticated_users
...
SELECT 
    gen_random_uuid(),
    'admin_support',
    ARRAY[c.id, (SELECT id FROM "Admin" ORDER BY created_at LIMIT 1)],
...
SELECT 
    gen_random_uuid(),
    'admin_support',
    ARRAY[e.id, (SELECT id FROM "Admin" ORDER BY created_at LIMIT 1)],
