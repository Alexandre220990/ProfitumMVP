-- Script simplifié pour créer les conversations admin
...
SELECT 
    gen_random_uuid(),
    'admin_support',
    ARRAY[c.id, (SELECT id FROM authenticated_users WHERE role = 'admin' LIMIT 1)],
    'Support Administratif - ' || COALESCE(c.name, c.email),
    'Conversation automatique pour le support administratif du client',
    'active',
    c.id,
    (SELECT id FROM authenticated_users WHERE role = 'admin' LIMIT 1),
    'private',
    'medium',
    'support',
    NOW(),
...
SELECT 
    gen_random_uuid(),
    'admin_support',
    ARRAY[e.id, (SELECT id FROM authenticated_users WHERE role = 'admin' LIMIT 1)],
...
SELECT 'Conversations admin créées' as status, COUNT(*) as count FROM conversations WHERE type = 'admin_support'; 