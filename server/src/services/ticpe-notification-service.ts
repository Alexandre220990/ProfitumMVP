...
export class TICPENotificationService {
  ...
  async notifyEligibilityConfirmed(data: TICPEWorkflowData): Promise<string> {
    return NotificationService.sendSystemNotification({
      ...
      message: `Votre éligibilité au remboursement TICPE a été confirmée. Montant estimé : ${data.estimated_amount}€`,
      type: 'system',
      ...data,
  ...
  async notifyExpertSelected(data: TICPEWorkflowData): Promise<string> {
    ...
    await NotificationService.sendSystemNotification({
      ...
      message: `${data.expert_name} a été sélectionné pour votre dossier TICPE.`,
      type: 'system',
      ...data,
      dashboard_url: `${process.env.FRONTEND_URL}/dashboard/client-audit/${data.dossier_id}`
    });
    ...
    if (data.expert_id) {
      await NotificationService.sendSystemNotification({
        ...
        title: 'Nouveau dossier TICPE assigné',
        message: `Vous avez été assigné au dossier TICPE de ${data.client_name} (${data.company_name}).`,
        type: 'system',
        ...data,
  ...
  async notifyDocumentsCollected(data: TICPEWorkflowData): Promise<string> {
    return NotificationService.sendSystemNotification({
      ...
      message: `${data.documents_count} documents ont été collectés pour votre dossier TICPE.`,
      type: 'system',
      ...data,
      dashboard_url: `${process.env.FRONTEND_URL}/dashboard/client-audit/${data.dossier_id}`
    });
  ...
  async notifyAuditCompleted(data: TICPEWorkflowData): Promise<string> {
    return NotificationService.sendSystemNotification({
      ...
      message: 'L\'audit comptable de votre dossier TICPE est terminé.',
      type: 'system',
      ...data,
      dashboard_url: `${process.env.FRONTEND_URL}/dashboard/client-audit/${data.dossier_id}`
    });
  ...
  async notifyValidationApproved(data: TICPEWorkflowData): Promise<string> {
    return NotificationService.sendSystemNotification({
      ...
      message: 'Votre dossier TICPE a été validé avec succès.',
      type: 'system',
      ...data,
      dashboard_url: `${process.env.FRONTEND_URL}/dashboard/client-audit/${data.dossier_id}`
    });
  ...
  async notifyAdminDocumentUploaded(data: TICPEWorkflowData): Promise<string> {
    ...
    for (const adminId of adminIds) {
      await NotificationService.sendSystemNotification({
        user_id: adminId,
        title: 'Document TICPE uploadé',
        message: `Nouveau document uploadé pour le dossier TICPE de ${data.client_name}.`,
        type: 'system',
        ...data,
