/**
 * Service d'agrégation de notifications par client
...
export class NotificationAggregationService {
   ...
   * À exécuter après création de notifications enfants
   */
  static async aggregateNotificationsByClient(adminUserId: string): Promise<void> {
    try {
      console.log(`📊 [Aggregation] Début agrégation notifications pour admin ${adminUserId}`);

      // 1. Récupérer toutes les notifications enfants non agrégées pour cet admin
      ...
      if (childError) {
        console.error('❌ [Aggregation] Erreur récupération notifications enfants:', childError);
        return;
      }

      if (!childNotifications || childNotifications.length === 0) {
        console.log('ℹ️  [Aggregation] Aucune notification à agréger');
        return;
      }

      console.log(`📊 [Aggregation] ${childNotifications.length} notification(s) trouvée(s)`);

      // 2. Grouper par client_id
      const groupedByClient = this.groupNotificationsByClient(childNotifications);

      console.log(`📊 [Aggregation] ${Object.keys(groupedByClient).length} client(s) avec actions`);
      ...
      for (const [clientId, actions] of Object.entries(groupedByClient)) {
        await this.createOrUpdateParentNotification(adminUserId, clientId, actions);
      }

      console.log('✅ [Aggregation] Agrégation terminée avec succès');
    } catch (error) {
      console.error('❌ [Aggregation] Erreur lors de l\'agrégation:', error);
    }
  }
  ...
  private static groupNotificationsByClient(notifications: any[]): Record<string, ClientPendingActions> {
    ...
    for (const notif of notifications) {
      ...
      grouped[clientId].dossiers.push({
        dossier_id: notif.action_data?.client_produit_id || notif.action_data?.dossier_id,
  ...
  private static async createOrUpdateParentNotification(
    ...
    try {
      ...
      const { data: existingParent } = await supabase
        ...
        .select('id, metadata')
        .eq('user_id', adminUserId)
        .eq('notification_type', 'client_actions_summary')
      ...
      const title = `📋 ${actions.client_company} - ${dossiersCount} dossier${dossiersCount > 1 ? 's' : ''} à traiter`;
      ...
      if (existingParent) {
        ...
        if (updateError) {
          console.error(`❌ [Aggregation] Erreur mise à jour parent pour client ${clientId}:`, updateError);
        } else {
          console.log(`✅ [Aggregation] Parent mis à jour pour client ${actions.client_company} (${dossiersCount} dossiers)`);
        }
      } else {
        ...
        if (createError) {
          console.error(`❌ [Aggregation] Erreur création parent pour client ${clientId}:`, createError);
        } else {
          console.log(`✅ [Aggregation] Parent créé pour client ${actions.client_company} (${dossiersCount} dossiers)`);

          // Lier les notifications enfants à cette parent
          await this.linkChildrenToParent(adminUserId, clientId, newParent.id);
        }
      }
    } catch (error) {
      console.error(`❌ [Aggregation] Erreur pour client ${clientId}:`, error);
    }
  }
  ...
  private static async linkChildrenToParent(
    ...
    try {
      ...
      if (linkError) {
        console.error(`❌ [Aggregation] Erreur liaison enfants au parent:`, linkError);
      } else {
        console.log(`✅ [Aggregation] ${children.length} enfant(s) lié(s) au parent`);
      }
    } catch (error) {
      console.error('❌ [Aggregation] Erreur lors de la liaison enfants:', error);
    }
  }
  ...
  static async cleanupOrphanParents(): Promise<void> {
    try {
      console.log('🧹 [Aggregation] Nettoyage des parents orphelins...');
      ...
      for (const parent of parents) {
        ...
        if (count === 0) {
          ...
          console.log(`✅ [Aggregation] Parent orphelin archivé: ${parent.id}`);
        }
      }

      console.log('✅ [Aggregation] Nettoyage terminé');
    } catch (error) {
      console.error('❌ [Aggregation] Erreur lors du nettoyage:', error);
    }
  }

  /**
   * Recalculer tous les parents pour tous les admins
   */
  static async recalculateAllParents(): Promise<void> {
    try {
      console.log('🔄 [Aggregation] Recalcul de tous les parents...');
      ...
      for (const admin of admins) {
        await this.aggregateNotificationsByClient(admin.auth_user_id);
      }

      // Nettoyer les orphelins
      await this.cleanupOrphanParents();

      console.log('✅ [Aggregation] Recalcul terminé pour tous les admins');
    } catch (error) {
      console.error('❌ [Aggregation] Erreur lors du recalcul:', error);
    }
  }
}
