...
router.get('/dossiers', authenticateUser, async (req, res) => {
  try {
    ...
    const { data: admin, error: adminError } = await supabase
      .from('Admin')
      .select('*')
      .eq('id', (req as RequestWithUser).user?.id)
      .single();
...
router.get('/dossiers/stats', authenticateUser, async (req, res) => {
  try {
    // Vérifier que l'utilisateur est admin
    const { data: admin, error: adminError } = await supabase
      .from('Admin')
...
router.get('/dossiers/:id', authenticateUser, async (req, res) => {
  try {
    const { id } = req.params;
...
router.put('/dossiers/:id', authenticateUser, async (req, res) => {
  try {
    const { id } = req.params;
