...
export function useDashboardClientEffects() {
  ...
  useEffect(() => {
    if (user?.id && user.id.length > 10 && user.id.includes('-')) {
  ...
  useEffect(() => {
    ...
    const hasVisited = localStorage.getItem(`hasVisitedDashboard_${user.id}`);
