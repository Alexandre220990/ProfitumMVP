...
export function useExperts(options?: { specialization?: string; produitId?: string }) {
    ...
    const fetchExperts = async () => {
      try {
        ...
        if (options?.produitId) {
          params.append('produitId', options.produitId);
        }
        ...
        const message = err instanceof Error ? err.message : 'Erreur lors de la récupération des experts';
        setError(message);
        toast({
          title: "Erreur",
          description: message,
