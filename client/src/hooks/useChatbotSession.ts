 ...
 */
export const useChatbotSession = (initialSimulationId: string | null = null) => {
   ...
   */
  const loadSession = (): ChatbotState | null => {
    try {
   ...
   */
  const saveSession = (stateToSave: ChatbotState) => {
    try {
   ...
   */
  const addMessage = (message: Omit<Message, 'timestamp' | 'created_at' | 'updated_at'>) => {
    setState(prevState => ({
      ...prevState,
      messages: [
        ...prevState.messages,
