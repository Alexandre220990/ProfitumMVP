...
interface RecentSimulationResponse { recentSimulation: Simulation | null; }

interface ProductsResponse { products: ClientProduitEligible[]; }

export const useSimulation = () => { const { user } = useAuth();
  ...
  const getOrCreateSimulation = async (clientId: string): Promise<Simulation | null> => { try {
      const response = await get<ApiResponse<RecentSimulationResponse>>(
        `/api/simulations/check-recent/${clientId }`
      );
      
      const data = extractData(response);
      if (data && 'recentSimulation' in data) { return data.recentSimulation as Simulation | null; }
  ...
  // Initialisation
  useEffect(() => { const initSimulation = async () => {
      if (!user?.id) return;
      ...
      if (simulationIdFromUrl) {
        ...
        try {
          ...
          const data = extractData(response);
          if (data) { setLocalAnswers(data as unknown as Record<number, string[]>); }
        } catch (error) { console.error("Erreur chargement réponses: ", error); }
      } else { const sim = await getOrCreateSimulation(user.id);
        if (sim?.id) {
          setSimulationId(String(sim.id));
          const newUrl = `/simulateur?simulationId=${sim.id }`;
          if (window.location.search !== `?simulationId=${ sim.id }`) { window.history.pushState({ }, "", newUrl);
          }
        }
      }
    };
  ...
  useEffect(() => { const loadQuestions = async () => {
      try {
        ...
        if (validQuestions.length === 0) { throw new Error("Format des questions invalide"); }
        ...
        toast({
          title: "Erreur", description: "Impossible de charger les questions", variant: "destructive" });
  ...
  useEffect(() => { if (Object.keys(localAnswers).length > 0 && simulationId) {
  ...
  useEffect(() => { if (simulationId) {
      const saved = localStorage.getItem(`simulation_${simulationId }_answers`);
      if (saved) { try {
          setLocalAnswers(JSON.parse(saved)); } catch (error) { console.error("Erreur parse réponses localStorage", error); }
      }
    }
  }, [simulationId]);

  // Gestion des réponses
  const handleSelect = async (value: string) => { if (!simulationId || !questions[currentStep]) return;
    ...
    setLocalAnswers((prev) => ({
      ...prev, [question.id]: updatedAnswers }));

    if (currentStep < questions.length - 1) { setCurrentStep((prev) => prev + 1); }
  };

  // Soumission finale
  const handleSubmit = async () => { if (!simulationId || !user?.id) return;

    try {
      // 1. Enregistrer les réponses
      await post(`/api/simulations/${simulationId }/answers`, { answers: localAnswers });
      ...
      const data = extractData(response);
      if (data && 'products' in data) { setEligibleProducts(data.products as ClientProduitEligible[]);
        setShowResults(true); } else { toast({
          title: "Erreur", description: "Analyse des réponses impossible", variant: "destructive" });
      }
    } catch (error) { console.error("Erreur soumission simulation: ", error);
      toast({
        title: "Erreur", description: "Erreur lors de la soumission", variant: "destructive" });
    }
  };

  return { questions, currentStep, localAnswers, isLoading, showResults, eligibleProducts, handleSelect, handleSubmit };
}; 