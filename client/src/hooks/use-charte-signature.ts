...
interface UseCharteSignatureReturn { isSigned: boolean;
  isLoading: boolean;
  isSigning: boolean;
 ...
 */
export const useCharteSignature = (clientProduitEligibleId?: string): UseCharteSignatureReturn => { console.log('🔄 Hook useCharteSignature appelé - VERSION MISE À JOUR: ', new Date().toISOString());
   ...
   */
  const checkSignature = useCallback(async () => { console.log('🔍 checkSignature appelée');
    ...
    try { console.log('🔍 Début de la vérification - setIsLoading(true)');
      setIsLoading(true);
      ...
      if (response.success && response.data) {
        ...
        if (response.data.signed) {
          console.log('✅ Signature trouvée: ', response.data.signature?.id); } else { console.log('ℹ️ Aucune signature trouvée'); }
      } else { console.error('❌ Erreur lors de la vérification: ', response.message);
        toast({
          title: "Erreur", description: response.message || "Impossible de vérifier la signature", variant: "destructive" });
      }
    } catch (error) { console.error('❌ Erreur lors de la vérification de la signature: ', error);
      toast({
        title: "Erreur", description: "Impossible de vérifier la signature", variant: "destructive" });
    } finally { console.log('🔍 Fin de la vérification - setIsLoading(false)');
      setIsLoading(false); }
   ...
   * Signe la charte
   */
  const handleSignCharte = useCallback(async (): Promise<boolean> => { if (!effectiveClientProduitEligibleId) {
      console.error('❌ Aucun clientProduitEligibleId fourni pour signer la charte');
      toast({
        title: "Erreur", description: "Impossible de signer la charte", variant: "destructive" });
      return false;
    }

    try { setIsSigning(true);
      ...
      if (response.success && response.data) { setIsSigned(true);
        setSignature(response.data);
        
        console.log('✅ Charte signée avec succès: ', response.data.id);
        
        toast({
          title: "Succès", description: "Charte signée avec succès" });

        return true;
      } else { console.error('❌ Erreur lors de la signature: ', response.message);
        
        toast({
          title: "Erreur", description: response.message || "Impossible de signer la charte", variant: "destructive" });

        return false;
      }
    } catch (error) { console.error('❌ Erreur lors de la signature de la charte: ', error);
  ...
  useEffect(() => { console.log('🔄 useEffect déclenché');
    ...
    if (effectiveClientProduitEligibleId) {
      ...
      checkSignature(); } else { console.log('⚠️ Pas d\'ID, pas d\'appel à checkSignature');
      setIsLoading(false); }
  }, [effectiveClientProduitEligibleId, checkSignature]); // Inclure checkSignature dans les dépendances

  return { isSigned, isLoading, isSigning, signature, signCharte: handleSignCharte, checkSignature };
}; 