...
export default function DetailsDossier() {
  ...
  const progressionPourcentage = (dossier.progression.filter((e: any) => e.completed).length / dossier.progression.length) * 100;
          ...
          <Card className="shadow-lg bg-white rounded-xl">
            <CardContent className="p-8 space-y-6">
              ...
              <div className="text-gray-600 space-y-2">
                ...
                <p className="flex items-center gap-2"><FileText className="w-5 h-5 text-gray-500" /> Documents: {dossier.documents.join(", ")}</p>
                <p className="flex items-center gap-2"><Phone className="w-5 h-5 text-gray-500" /> Contact: <a href={`mailto:${dossier.contact.email}`} className="text-blue-600 font-medium">{dossier.contact.email}</a> | {dossier.contact.tel}</p>
              </div>
            </CardContent>
          </Card>
