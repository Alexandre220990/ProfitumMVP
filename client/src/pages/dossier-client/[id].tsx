...
import { Building, Briefcase, Users, MapPin, Mail, Phone, FileText, Search, Target, Truck, Fuel, DollarSign, ClipboardList, MessageSquare, Eye, Download, UserCheck, XCircle } from "lucide-react";
...
const ClientFile = () => { const { id } = useParams();
  ...
  const incomingRequests = [
    { id: 1, status: "pending" as DossierStatus, clientName: "Paul Lambert", companyName: "Transports Lambert", sector: "Transport routier de marchandises", employees: 45, address: "12 Rue des Routiers, 75015 Paris", email: "paul.lambert@email.com", phone: "+33 6 45 78 12 34", auditType: "Audit TICPE", creationDate: "01/03/2025", context: "Paul Lambert souhaite optimiser la récupération de la TICPE et s'assurer de la conformité fiscale sur les 3 dernières années.", objectives: [
        "Vérification des montants remboursables", "Identification des optimisations possibles", "Sécurisation des déclarations pour éviter tout redressement", "Conseil sur les stratégies futures pour réduire les coûts"
      ], fleetSize: "12 camions (Euro 5 & Euro 6)", annualConsumption: "320 000 litres de gazole", previousRefund: "21 500 € (2023)", documents: [
  ...
  const activeFiles = [
    { id: 6, status: "accepted" as DossierStatus, clientName: "Jean Dupont", companyName: "Dupont Transports", sector: "Transport routier", employees: 30, address: "15 Rue des Marchands, 69003 Lyon", email: "jean.dupont@email.com", phone: "+33 6 12 34 56 78", auditType: "Audit TICPE", creationDate: "10/01/2025", fleetSize: "8 camions (Euro 6)", annualConsumption: "180 000 litres de gazole", documents: [
  ...
  useEffect(() => { // Simulation de la récupération des données
    const fetchDossier = () => {
      setLoading(true);
  ...
  if (!dossier) { return <div className="flex justify-center items-center min-h-screen">Dossier non trouvé</div>; }

  const handleAccept = () => { console.log("Dossier accepté:", dossier.id);
    // Logique d'acceptation à implémenter
    navigate("/dashboard/partner"); };

  const handleReject = () => { console.log("Dossier refusé:", dossier.id);
