...
const ApporteurEventSynthese: React.FC = () => {
  ...
  const loadEventData = async () => {
    setLoading(true);
    try {
      const response = await get<EventSyntheseResponse>(`/apporteur/events/${id}/synthese`);
  ...
  if (!user || user.type !== 'apporteur') {
    return <Navigate to="/apporteur/login" replace />;
  }
...
export default ApporteurEventSynthese;
