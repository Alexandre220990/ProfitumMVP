    ...
    return (<div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="relative space-y-6">
          {processSteps.map(({ id, title, component: Component, description }) => (<Card key={id} className={`relative p-6 transition-all ${id === currentStep ? 'border-blue-500 bg-blue-50 shadow-md' : id < currentStep ? 'border-green-500 bg-green-50' : 'border-gray-300 bg-gray-100'}`}>
        ...
        <div className="fixed bottom-6 right-6 flex gap-4">
          {currentStep > 1 && (<Button variant="ghost" onClick={handleStepBack}>
              <ArrowLeftCircle className="w-8 h-8 text-blue-600"/>
            </Button>)}
          {currentStep < processSteps.length && (<Button variant="ghost" onClick={handleStepCompletion}>
