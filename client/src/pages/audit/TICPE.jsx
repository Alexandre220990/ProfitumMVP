        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-6">
              {processSteps.map(({ id, title, component: Component, description }) => (<div key={id} className={`p-4 border-2 rounded-lg transition-all transform hover:scale-105 ${id === currentStep
                ...
                : id < currentStep
                  ...
                  {id === currentStep && (<div className="flex justify-start mb-4">
                  ...
                  {id === 1 && currentStep === 1 && (<>
                      <div className="flex gap-4">
                        ...
                        <Button onClick={() => handleStepCompletion(id)} disabled={!charterValidated}>
                          Valider la charte
                        </Button>
                      </div>
                      <CharterDialog open={charterOpen} onClose={() => {
                  ...
                  {id === 2 && currentStep === 2 && (<>
                      ...
                      <ExpertSelection open={expertOpen} onClose={() => {
                    setExpertOpen(false);
                    handleStepCompletion(id);
                }} auditType={auditType}/>
                    </>)}
                  {id === currentStep && Component && id !== 2 && <Component onComplete={() => handleStepCompletion(id)}/>}
                </div>))}
            </div>
          </CardContent>
        </Card>
