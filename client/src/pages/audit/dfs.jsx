...
import { Check, Circle, Calendar, Mail, Download, Trash2 } from "lucide-react";
...
const StepIndicator = ({ status, number }) => {
    return (<div className={cn("w-8 h-8 rounded-full flex items-center justify-center", status === "completed" && "bg-green-100 text-green-600", status === "current" && "bg-orange-100 text-orange-600", status === "upcoming" && "bg-gray-100 text-gray-600")}>
      {status === "completed" ? (<Check className="w-4 h-4"/>) : (<span className="font-semibold">{number}</span>)}
    </div>);
};
export default function DFSAudit() {
    ...
    const [uploadedDocuments, setUploadedDocuments] = useState(documentsList);
    useEffect(() => {
        ...
        if (Object.keys(savedDocuments).length > 0) {
            setUploadedDocuments(prevDocs => prevDocs.map(doc => (Object.assign(Object.assign({}, doc), { uploadedFiles: savedDocuments[doc.id] ? savedDocuments[doc.id].map(f => ({ id: f.id, name: f.name })) : [] }))));
        }
    }, [auditType]);
    ...
    const handleDocumentUpload = (docId, files) => {
        const updatedDocs = uploadedDocuments.map(d => {
            if (d.id === docId) {
                return Object.assign(Object.assign({}, d), { uploadedFiles: [...d.uploadedFiles, ...files] });
            }
            return d;
        });
        setUploadedDocuments(updatedDocs);
        localStorage.setItem(`${auditType}_documents`, JSON.stringify(updatedDocs.reduce((acc, d) => (Object.assign(Object.assign({}, acc), { [d.id]: d.uploadedFiles })), {})));
    ...
    const handleDeleteDocument = (docId, fileId) => {
        const updatedDocs = uploadedDocuments.map(doc => (Object.assign(Object.assign({}, doc), { uploadedFiles: doc.uploadedFiles.filter(file => file.id !== fileId) })));
        setUploadedDocuments(updatedDocs);
        localStorage.setItem(`${auditType}_documents`, JSON.stringify(updatedDocs.reduce((acc, d) => (Object.assign(Object.assign({}, acc), { [d.id]: d.uploadedFiles })), {})));
    };
    const steps = [
        {
            ...
            content: !isCharterSigned && currentStep === 1 && (<Button variant="outline" size="sm" className="mt-2" onClick={() => setShowCharterDialog(true)}>
        ...
        },
        {
            ...
            content: selectedExpert && (<div className="mt-4 space-y-2">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-medium">{selectedExpert.name}</p>
              <p className="text-sm text-gray-600">{selectedExpert.speciality}</p>
            </div>
            {!expertContactSuccess && (<Button size="sm" variant="outline" className="flex items-center gap-2" onClick={() => setShowCompensationDialog(true)}>
                <Mail className="w-4 h-4"/>
                Contact
              </Button>)}
          </div>
          {expertContactSuccess && (<p className="text-green-600 text-sm mt-2">
        ...
        },
        {
            ...
            content: expertContactSuccess && (<div className="mt-4">
          {confirmedDateTime ? (<p className="text-green-600 text-sm">{confirmationMessage}</p>) : (<Button variant="outline" onClick={() => setShowCalendarDialog(true)} className="flex items-center gap-2">
        ...
        },
        {
            ...
            content: currentStep >= 4 && (<div className="mt-4 space-y-3">
          {uploadedDocuments.map((doc) => (<div key={doc.id} className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                <span className={cn("text-sm flex-grow", doc.uploadedFiles.length > 0 && "text-green-600")}>
                  {doc.label}
                </span>
                <FileUpload documentId={doc.id} onUploadComplete={(files) => handleDocumentUpload(doc.id, files)}/>
              </div>
              {doc.uploadedFiles.length > 0 && (<div className="ml-4 text-xs text-green-600">
                  {doc.uploadedFiles.map((file) => (<div key={file.id} className="flex items-center justify-between gap-1.5">
                      <div className="flex items-center gap-1.5">
                        <Check className="w-3 h-3"/>
                        {file.name}
                      </div>
                      <Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-red-600 hover:text-red-700 hover:bg-red-50" onClick={() => handleDeleteDocument(doc.id, file.id)}>
    ...
    return (<div className="min-h-screen bg-gray-50 p-6">
          ...
          <Card>
            ...
            return (<div key={index} className="flex items-start gap-4">
                      ...
                      <div className="flex-grow">
                        <h3 className={cn("font-semibold", status === "completed" && "text-green-600", status === "current" && "text-orange-600", status === "upcoming" && "text-gray-900")}>
                          {step.title}
                        </h3>
                        <p className="text-sm text-gray-600 mb-2">{step.description}</p>
                        {step.content && (status === "current" || status === "completed") && step.content}
                        {step.action && (status === "current" || status === "completed") && (<Link href={step.action.href}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {experts.map((expert) => (<Link key={expert.id} href={`/audit/${auditType}/expert/${expert.id}`} onClick={() => {
      ...
      <CharterDialog open={showCharterDialog} onOpenChange={setShowCharterDialog} onAccept={handleCharterSign} auditType={auditType} isCharterSigned={isCharterSigned} setIsCharterSigned={setIsCharterSigned}/>
      ...
      <Dialog open={showCompensationDialog} onOpenChange={setShowCompensationDialog}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="p-4 bg-gray-50 rounded-lg">
              ...
              <p className="text-sm text-gray-600 mb-4">
                La rémunération de l'expert est fixée à {selectedExpert === null || selectedExpert === void 0 ? void 0 : selectedExpert.compensation}% des économies réalisées sur les trois premières années.
            ...
            <div className="flex items-center space-x-2">
              <Checkbox id="terms" checked={acceptedCompensation} onCheckedChange={(checked) => setAcceptedCompensation(checked)}/>
          ...
          <DialogFooter>
            <Button onClick={handleCompensationValidation} disabled={!acceptedCompensation}>
              Valider
            </Button>
          </DialogFooter>
        </DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="flex gap-6">
              <div className="flex-1">
                <CalendarComponent mode="single" selected={selectedDate} onSelect={setSelectedDate} locale={fr} disabled={(date) => date < new Date() || date > addDays(new Date(), 30)} className="rounded-md border" showOutsideDays={false} ISOWeek={true}/>
              </div>

              {selectedDate && (<div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 gap-2 max-h-[300px] overflow-y-auto p-2">
                    {getTimeSlots().map((time) => (<Button key={time} variant={selectedTime === time ? "default" : "outline"} onClick={() => setSelectedTime(time)} className="text-sm">
                        {time}
                      </Button>))}
                  </div>
                </div>)}
            ...
            <div className="flex justify-end items-center pt-4 border-t">
              <Button onClick={handleScheduleMeeting} disabled={!selectedDate || !selectedTime}>
                OK
              </Button>
            </div>
          </div>
