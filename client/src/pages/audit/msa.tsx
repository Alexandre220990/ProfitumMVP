    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex justify-center mt-6">
          {processSteps.map(({ id, component: Component, description }) => (
            id === currentStep && (
              <Card key={id} className="w-full p-8 text-center shadow-lg">
                ...
                {Component && id === 1 ? (
                  <>
                    <div className="flex flex-col items-center space-y-4">
                      ...
                      {charterValidated && (
                        <Button onClick={handleStepCompletion} className="mt-4 bg-green-500 text-white">
                          Valider la charte
                        </Button>
                      )}
                    </div>
        ...
        <div className="flex justify-between mt-6">
          {currentStep > 1 && (
            <Button variant="ghost" onClick={handleStepBack} className="w-24 h-24 flex items-center justify-center">
              <ArrowLeftCircle className="w-20 h-20 text-blue-600" />
            </Button>
          )}
          {currentStep < processSteps.length && (
            <Button variant="ghost" onClick={handleStepCompletion} className="w-16 h-16">
