              ...
              {processSteps.map(({ id, title, component: Component, description }) => (<div key={id} className={`p-4 border-2 rounded-lg transition-all transform hover:scale-105 ${id === currentStep
                ...
                : id < currentStep
                  ...
                  {id === 1 && currentStep === 1 && (<>
                      ...
                      <CharterDialog open={charterOpen} onScrollEnd={() => setCharterValidated(true)} onClose={() => setCharterOpen(false)}/>
                    </>)}
                  {id === currentStep && Component && <Component onComplete={() => handleStepCompletion(id)}/>}
                </div>))}
            </div>
          </CardContent>
        </Card>
