...
import { User, Settings, LogOut, MessageCircle, FileText, Briefcase, Users, Clock } from "lucide-react";
import ExpertNotificationCenter from "./ExpertNotificationCenter";

export default function HeaderExpert() { const { user, logout } = useAuth();
  const navigate = useNavigate();
  
  console.log("HeaderExpert - User: ", user);

  const handleLogout = async () => { // Utiliser la fonction logout du hook useAuth
    logout();
    ...
    <header className="bg-white shadow-md py-4 px-8 flex justify-between items-center fixed top-0 left-0 right-0 w-full z-50">
      ...
      <nav className="flex space-x-10 text-gray-700 font-semibold text-lg">
        ...
        <div onClick={ () => navigate("/demandes-en-attente") } 
          className="flex items-center space-x-2 hover:text-blue-600 transition-colors cursor-pointer">
          <Clock className="h-5 w-5" /> <span>Demandes</span>
        </div>
        <div onClick={ () => navigate("/clients-expert") } 
          className="flex items-center space-x-2 hover:text-blue-600 transition-colors cursor-pointer">
