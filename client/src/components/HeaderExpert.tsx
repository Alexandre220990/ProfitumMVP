...
import { User, Settings, LogOut, MessageCircle, FileText, Briefcase, Calendar, TrendingUp } from "lucide-react";
import ExpertNotificationCenter from "./ExpertNotificationCenter";

export default function HeaderExpert() {
  const { user, logout } = useAuth();
  const navigate = useNavigate();
  
  console.log("HeaderExpert - User: ", user);
  ...
  return (
    <header className="bg-white shadow-md py-4 px-8 flex justify-between items-center fixed top-0 left-0 right-0 w-full z-50">
      ...
      {/* NAVIGATION EXPERT - NOUVELLE STRUCTURE OPTIMISÉE */}
      <nav className="flex space-x-8 text-gray-700 font-semibold text-lg">
        ...
        <div onClick={() => navigate("/expert/agenda")} 
          className="flex items-center space-x-2 hover:text-blue-600 transition-colors cursor-pointer">
          <Calendar className="h-5 w-5" /> 
          <span>Agenda</span>
        </div>
        
        <div onClick={() => navigate("/expert/messagerie-expert")} 
          className="flex items-center space-x-2 hover:text-blue-600 transition-colors cursor-pointer">
          <MessageCircle className="h-5 w-5" /> 
          <span>Messagerie</span>
        </div>
        
        <div onClick={() => navigate("/expert/mes-affaires")} 
          className="flex items-center space-x-2 hover:text-blue-600 transition-colors cursor-pointer">
          <TrendingUp className="h-5 w-5" /> 
          <span>Mes affaires</span>
        </div>

        <div onClick={() => navigate("/expert/analytics")} 
          className="flex items-center space-x-2 hover:text-blue-600 transition-colors cursor-pointer">
          <TrendingUp className="h-5 w-5" /> 
          <span>Analytics</span>
        </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56 shadow-lg border rounded-lg">
            ...
            <DropdownMenuItem onClick={() => navigate("/expert/documents")} 
              className="flex items-center px-4 py-2 hover:bg-gray-100 transition cursor-pointer">
              <FileText className="mr-2 h-5 w-5" /> 
              <span>Documents</span>
            </DropdownMenuItem>
            
            <DropdownMenuItem onClick={() => navigate("/expert/analytics")} 
              className="flex items-center px-4 py-2 hover:bg-gray-100 transition cursor-pointer">
