...
export interface RegisterCredentials extends Partial<ClientData>, Partial<ExpertData> {
  email: string;
  password: string;
  type: "client" | "expert" | "admin";
  name?: string;
...
export interface CharteSignature {
  id: string;
  client_id: string;
  produit_id: string;
...
export interface CharteSignatureRequest {
  clientProduitEligibleId: string;
  ipAddress?: string;
  userAgent?: string;
}

export interface CharteSignatureResponse {
  signed: boolean;
