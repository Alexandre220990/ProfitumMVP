...
interface FileUploadProps { auditId?: number;
  documentId?: string;
...
export default function FileUpload({ auditId, documentId, onUploadComplete }: FileUploadProps) { 
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => { if (e.target.files) {
      const selectedFiles = Array.from(e.target.files) as File[];
      await uploadFiles(selectedFiles); }
  };

  const uploadFiles = async (selectedFiles: File[]) => { if (selectedFiles.length === 0) return;
    ...
    try {
      ...
      if (auditId) { formData.append("auditId", auditId.toString()); }
      ...
      onUploadComplete?.(uploadedFiles.map((file: any) => ({ id: file.id, name: file.filename || file.name })));
    } catch (error: any) { toast({
        variant: "destructive", title: "Erreur", description: error.message || "Erreur lors du téléchargement des fichiers" });
