...
async function createExpertAlexandre() {
  console.log('👨‍💼 Création de l\'expert Alexandre complet...\n');

  try {
    // 1. Supprimer l'utilisateur existant s'il existe
    console.log('📝 Étape 1: Nettoyage des données existantes...');
    
    // Vérifier si l'utilisateur existe dans Auth
    const { data: authUsers, error: authError } = await supabase.auth.admin.listUsers();
    if (!authError) {
      const existingUser = authUsers.users.find(user => user.email === 'alexandre@profitum.fr');
      if (existingUser) {
    ...
    // Supprimer l'expert existant dans la table Expert
    ...
    // 2. Créer un nouvel utilisateur Supabase Auth
    console.log('📝 Étape 2: Création de l\'utilisateur Supabase Auth...');
    ...
    // 3. Créer l'expert dans la table Expert
    console.log('📝 Étape 3: Création de l\'expert dans la table Expert...');
    const expertData = {
    ...
    // 4. Créer des données de test pour le dashboard expert
    console.log('📝 Étape 4: Création des données de test pour le dashboard...');
    
    // Créer quelques clients de test
    const testClients = [
      {
        id: 'client-test-alex-1',
        ...
        statut: 'actif',
        created_at: new Date().toISOString(),
        auth_id: 'auth-client-alex-1'
      },
      {
        id: 'client-test-alex-2',
        email: 'logistique@test.fr',
        company_name: 'Logistique Pro',
        city: 'Marseille',
        phone: '+33 4 98 76 54 32',
        statut: 'actif',
        created_at: new Date().toISOString(),
        auth_id: 'auth-client-alex-2'
      }
    ];
    ...
    const assignments = [
      {
        id: 'assignment-alex-1',
        expert_id: expert.id,
        client_id: 'client-test-alex-1',
        status: 'accepted',
      ...
      {
        id: 'assignment-alex-2',
        expert_id: expert.id,
        client_id: 'client-test-alex-2',
        status: 'pending',
        assignment_date: new Date().toISOString(),
    ...
    // 5. Créer des notifications de test
    const notifications = [
      {
        id: 'notif-alex-1',
        user_id: expert.id,
