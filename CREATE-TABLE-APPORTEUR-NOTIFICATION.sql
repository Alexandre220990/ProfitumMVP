...
BEGIN
    ...
    IF table_exists THEN
        RAISE NOTICE 'ℹ️ Table ApporteurNotification existe déjà - Pas de création nécessaire';
    ELSIF view_exists THEN
        RAISE NOTICE '⚠️ Une vue ApporteurNotification existe - Elle sera supprimée et remplacée par la table';
        DROP VIEW IF EXISTS "ApporteurNotification" CASCADE;
    ELSE
        RAISE NOTICE '✅ ApporteurNotification n''existe pas - Création nécessaire';
    END IF;
END $$;

-- ============================================================================
-- ÉTAPE 2: CRÉER LA TABLE ApporteurNotification
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS "ApporteurNotification" (
  ...
  -- Type et contenu
  type TEXT NOT NULL, -- 'prospect_assigned', 'client_created', 'dossier_updated', etc.
  title TEXT NOT NULL,
  ...
  -- Métadonnées
  metadata JSONB, -- Données additionnelles (client_id, prospect_id, dossier_id, etc.)
  ...
  -- Apporteur qui a traité
  handled_by UUID, -- ID de l'apporteur qui a traité la notification
...
-- Index sur handled_by (pour trouver les notifications traitées par un apporteur)
...
-- Trigger pour initialiser les statuts dans ApporteurNotificationStatus (si la table existe)
CREATE OR REPLACE FUNCTION initialize_apporteur_notification_status()
RETURNS TRIGGER AS $$
BEGIN
    -- Créer une entrée de statut pour chaque apporteur actif
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' 
        AND table_name = 'ApporteurNotificationStatus'
    ) THEN
        INSERT INTO "ApporteurNotificationStatus" (notification_id, apporteur_id, is_read, is_archived)
        SELECT NEW.id, a.id, FALSE, FALSE
        FROM "Apporteur" a
        WHERE a.is_active = TRUE
        ON CONFLICT (notification_id, apporteur_id) DO NOTHING;
    END IF;
...
-- Créer le trigger seulement si ApporteurNotificationStatus existe
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        ...
        DROP TRIGGER IF EXISTS trg_initialize_apporteur_notification_status ON "ApporteurNotification";
        CREATE TRIGGER trg_initialize_apporteur_notification_status
            AFTER INSERT ON "ApporteurNotification"
            FOR EACH ROW
            EXECUTE FUNCTION initialize_apporteur_notification_status();
        
        RAISE NOTICE '✅ Trigger trg_initialize_apporteur_notification_status créé';
    ELSE
        RAISE NOTICE 'ℹ️ Table ApporteurNotificationStatus n''existe pas - Trigger non créé';
    END IF;
END $$;
...
-- ============================================================================
-- ÉTAPE 5: CRÉER LA VUE ApporteurNotificationActive
-- ============================================================================
...
CREATE OR REPLACE FUNCTION cleanup_old_apporteur_notifications()
RETURNS void AS $$
BEGIN
  ...
  DELETE FROM "ApporteurNotification"
  WHERE 
    status = 'archived' 
...
COMMENT ON COLUMN "ApporteurNotification".type IS 'Type: prospect_assigned, client_created, dossier_updated, etc.';
COMMENT ON COLUMN "ApporteurNotification".status IS 'Statut: pending, read, archived';
COMMENT ON COLUMN "ApporteurNotification".priority IS 'Priorité: low, normal, high, urgent';
COMMENT ON COLUMN "ApporteurNotification".metadata IS 'Données JSON (client_id, prospect_id, dossier_id, etc.)';
COMMENT ON COLUMN "ApporteurNotification".action_url IS 'URL pour action rapide';
COMMENT ON COLUMN "ApporteurNotification".action_label IS 'Label du bouton action';
...
BEGIN
    ...
    IF NOT table_exists THEN
        RAISE EXCEPTION '❌ ERREUR: Table ApporteurNotification non créée';
    END IF;
